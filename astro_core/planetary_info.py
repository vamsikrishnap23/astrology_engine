...
def compute_planetary_info_telugu(year, month, day, hour, minute, second, lat, lon, tz_offset):
    ...
    rahu_rasi = get_rasi(rahu_long)
    rahu_deg_in_sign = rahu_long % 30
    rahu_nakshatra = get_nakshatra(rahu_long)
    rahu_pada = get_pada(rahu_long)
    info.append({
        "planet": "Rahu",
    ...
    ketu_rasi = get_rasi(ketu_long)
    ketu_deg_in_sign = ketu_long % 30
    ketu_nakshatra = get_nakshatra(ketu_long)
    ketu_pada = get_pada(ketu_long)
    info.append({
        "planet": "Ketu",
